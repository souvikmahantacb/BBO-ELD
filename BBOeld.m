function [MinCost, Hamming] = BBOeld(eld, DisplayFlag, ProbFlag, RandSeed)
...
[OPTIONS, MinCost, AvgCost, InitFunction, CostFunction,FeasibleFunction, Population] = Init2(DisplayFlag, RandSeed);
...
for GenIndex = 1 : OPTIONS.Maxgen
        ...
        for k = round(length(Population)/2) : length(Population)
            ...
            for parnum = 1 : OPTIONS.numVar
                if MutationRate(k) > rand
                    Island(k,parnum) =OPTIONS.pgmin(j)+rand*(OPTIONS.pgmax(j)-OPTIONS.pgmin(j));
                end
            end
        end
    ...
    % *********************************************************************
                % Calculation of Each Steps with New Modified Set
    % *********************************************************************
    ...
    [AverageCost, nLegal] = ComputeAveCost(Population);
     Population(1).chrom
    Population(1).cost1 = 0;
    for i=1:OPTIONS.numVar
            Population(1).cost1=Population(1).cost1+OPTIONS.alpha(i)*(Population(1).chrom(i))^2+OPTIONS.beta(i)*(Population(1).chrom(i))+OPTIONS.gama(i);
% Population(1).cost1=Population(1).cost1+abs(OPTIONS.sin1(i)*sin(OPTIONS.sin2(i)*(-Population(1).chrom(i)+OPTIONS.pgmin(i))));
    end
  
     ploss=0;
   sumGen=sum(Population(1).chrom);
 ploss=sumGen-OPTIONS.pdemand;


  cost(i)=0;
cost1=Population(1).cost1;
%emission=Population(1).emission;
% h= 0.0451646;
% Population(1).cost=Population(1).cost1+h*Population(1).emission;

fprintf(" In iteration %d, best fuel cost is %10.4f and power loss is %10.6f", GenIndex, cost1, ploss);
